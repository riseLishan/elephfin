...
public class DownloadApk {
    ...
    public static void downloadApk(Context context, String url, String title,final String appName) {
        ...
        //获取存储的下载ID
        long downloadId = SystemInfo.getInstance().getLong(DownloadManager.EXTRA_DOWNLOAD_ID,-1L);
        if(downloadId != -1) {
            //存在downloadId
            ...
            if(DownloadManager.STATUS_SUCCESSFUL == status) {
                ...
                Log.d(context.getPackageName(), "apk已经下载");
            }
        } else {
    ...
    private static void start(Context context, String url, String title,String appName) {
        ...
        if(hasSDKCard()) {
            ...
            SystemInfo.getInstance().setLong(DownloadManager.EXTRA_DOWNLOAD_ID,id);
        } else {
    ...
    public static void registerBroadcast(Context context) {
        ...
        context.registerReceiver(installReceiver, new IntentFilter(DownloadManager.ACTION_DOWNLOAD_COMPLETE));
    }
    ...
    public static void removeFile(Context context) {
        String filePath = SystemInfo.getInstance().getString("downloadApk",null);
        if(null != filePath) {
